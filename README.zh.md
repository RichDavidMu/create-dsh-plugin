...
- **版本号就是契约。** `@rdmu/create-dsh-plugin@0.1.2-rc.1.rev.1` 生成的项目精确
  钉死 `@deepseek-ai/dsh-*@0.1.2-rc.1` —— 不带 caret。没有 `--dsh-version` 参数，
  因为一个事实不该有两处真相。
...
`@rdmu/create-dsh-plugin@0.1.2-rc.1.rev.1` 生成的项目精确钉死
`@deepseek-ai/dsh-*@0.1.2-rc.1` —— 不带 caret。没有 `--dsh-version` 参数，因为那会让
一个事实有两处真相。
