...
```sh
pnpm create dsh-plugin my-plugin
cd my-plugin
pnpm install
pnpm run check
```
...
**This package's version IS the dsh version it targets.** `create-dsh-plugin@0.1.0-rc.7`
...
```sh
git clone https://github.com/muchenwei/create-dsh-plugin
pnpm install
...
unit tests cannot reach. The repository's `AGENTS.md` explains how the template is
split across `packages/example/plugin-hello`, `templates/`, and `docs/`, and why.
